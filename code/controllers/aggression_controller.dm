	...
	switch(asteroid_stage)
		if(1)
			for(var/mob/M in mob_targets)
				to_chat(M, pick("A steady vibration under your feet lets you know theirs more comey", "The ground lightly trembles below you, and you get a sense of dread."))
				M.playsound_local(M.loc, pick('sound/effects/earthquake_short.ogg','sound/effects/earthquake_short2.ogg'), 20, 0)
				shake_camera(M, 5, 2)
			var/forced_spawns = 2
			while(forced_spawns > 0)
...
/datum/aggression_machine/Process()
	if(round_duration_in_ticks > checkbuffer)
		...
		check_dead()
		trigger_event()	
	if(round_duration_in_ticks > wavebuffer)
		wavebuffer = round_duration_in_ticks + rand(45 SECONDS, 75 SECONDS)
		check_targets()
		switch(asteroid_stage)
			if(0)

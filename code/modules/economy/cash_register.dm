	...
	if(href_list["choice"])
		switch(href_list["choice"])
			...
			if("custom_order")
				...
				src.visible_message("\icon[src][transaction_purpose]: [t_amount] Ethericoin\s.")
			if("set_amount")
...
/obj/machinery/cash_register/proc/confirm(obj/item/I)
	if(confirm_item == I)
		...
		confirm_item = I
		src.visible_message("\icon[src]<b>Total price:</b> [transaction_amount] Ethericoin\s. Swipe again to confirm.")
		playsound(src, 'sound/machines/twobeep.ogg', 25)
		return 0
...
/obj/machinery/cash_register/proc/scan_item_price(obj/O)
	...
	if(isnull(price))
		...
		return
	// Call out item cost
	src.visible_message("\icon[src]\A [O]: [price ? "[price] Ethericoin\s" : "free of charge"].")
	// Note the transaction purpose for later use
	if(transaction_purpose)
		transaction_purpose += "<br>"
	transaction_purpose += "[O]: [price] Ethericoin\s"

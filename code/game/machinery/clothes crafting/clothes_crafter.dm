/obj/machinery/clothes_crafter
	icon = 'icons/obj/forensics.dmi'
	icon_state = "fab-idle"
	name = "Mechanical Stitcher"
	desc = "For manufacturing clothing."
	density = 1
	anchored = 1
	use_power = 1
	idle_power_usage = 10
...
/obj/machinery/clothes_crafter/New()
	..()
...
/obj/machinery/clothes_crafter/Initialize()
	manufacturer = basic_robolimb.company
	update_categories()
	. = ..()

/obj/machinery/clothes_crafter/Process()
	..()
	if(stat)
		return
	if(busy)
...
/obj/machinery/clothes_crafter/update_icon()
	overlays.Cut()
	if(panel_open)
		icon_state = "fab-o"
	else
...
/obj/machinery/clothes_crafter/dismantle()
	for(var/f in materials)
		eject_materials(f, -1)
	..()

/obj/machinery/clothes_crafter/RefreshParts()
	res_max_amount = 0
...
/obj/machinery/clothes_crafter/attack_hand(var/mob/user)
	if(..())
		return
	if(!allowed(user))
		return
...
/obj/machinery/clothes_crafter/ui_interact(var/mob/user, var/ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	var/data[0]
...
/obj/machinery/clothes_crafter/Topic(href, href_list)
	if(..())
		return
...
/obj/machinery/clothes_crafter/attackby(var/obj/item/I, var/mob/user)
	if(busy)
...
/obj/machinery/clothes_crafter/proc/update_busy()
	if(queue.len)
...
/obj/machinery/clothes_crafter/proc/add_to_queue(var/index)
	var/datum/design/D = files.known_designs[index]
	queue += D
	update_busy()

/obj/machinery/clothes_crafter/proc/remove_from_queue(var/index)
	if(index == 1)
		progress = 0
	queue.Cut(index, index + 1)
	update_busy()

/obj/machinery/clothes_crafter/proc/can_build(var/datum/design/D)
	for(var/M in D.materials)
		if(materials[M] <= D.materials[M] * mat_efficiency)
			return 0
	return 1

/obj/machinery/clothes_crafter/proc/check_build()
	if(!queue.len)
		progress = 0
		return
...
/obj/machinery/clothes_crafter/proc/get_queue_names()
	. = list()
	for(var/i = 2 to queue.len)
		var/datum/design/D = queue[i]
		. += D.name

/obj/machinery/clothes_crafter/proc/get_build_options()
	. = list()
...
/obj/machinery/clothes_crafter/proc/get_design_resourses(var/datum/design/D)
	var/list/F = list()
	for(var/T in D.materials)
		F += "[capitalize(T)]: [D.materials[T] * mat_efficiency]"
	return english_list(F, and_text = ", ")

/obj/machinery/clothes_crafter/proc/get_design_time(var/datum/design/D)
	return time2text(round(10 * D.time / speed), "mm:ss")

/obj/machinery/clothes_crafter/proc/update_categories()
	categories = list()
...
/obj/machinery/clothes_crafter/proc/get_materials()
	. = list()
	for(var/T in materials)
		. += list(list("mat" = capitalize(T), "amt" = materials[T]))

/obj/machinery/clothes_crafter/proc/eject_materials(var/material, var/amount) // 0 amount = 0 means ejecting a full stack; -1 means eject everything
	var/recursive = amount == -1 ? 1 : 0
	material = lowertext(material)
	var/mattype
	switch(material)
        if("cloth")
			mattype = /obj/item/stack/material/cloth
        if("leather")
			mattype = /obj/item/stack/material/leather

...
/obj/item/organ/external/proc/get_fingerprint()
	...
	if(has_fingerprint() && dna && !is_stump() && !BP_IS_ROBOTIC(src))
...
/obj/item/organ/external/emp_act(severity)
	...
	switch (severity)
		...
		if (3)
			burn_damage = 3
	if(status & ORGAN_ROBOTIC || status & ORGAN_ASSISTED)
		burn_damage /= burn_mod //ignore burn mod for EMP damage
...
/obj/item/organ/external/proc/createwound(var/type = DAM_CUT, var/damage, var/surgical)
	...
	if((type in ORGAN_ENERGY_DAMAGES) && (damage > 5 || damage + burn_dam >= 15) && (!(status & ORGAN_ROBOTIC)))
...
/obj/item/organ/external/proc/need_process()
	...
	if(status & (ORGAN_CUT_AWAY|ORGAN_BLEEDING|ORGAN_BROKEN|ORGAN_DEAD|ORGAN_MUTATED))
		return 1
	if((brute_dam || burn_dam) && (!(status & ORGAN_ROBOTIC))) //Robot limbs don't autoheal and thus don't need to process when damaged
		return 1
...
/obj/item/organ/external/proc/update_germs()
	...
	if(status & ORGAN_ROBOTIC || (owner.species && owner.species.species_flags & SPECIES_FLAG_IS_PLANT)) //Robotic limbs shouldn't be infected, nor should nonexistant limbs.
		germ_level = 0
		return
...
/obj/item/organ/external/handle_germ_effects()
	...
	if(germ_level >= INFECTION_LEVEL_TWO)
		...
		if (children)
			for (var/obj/item/organ/external/child in children)
				if (child.germ_level < germ_level && !(child.status & ORGAN_ROBOTIC))
					if (child.germ_level < INFECTION_LEVEL_ONE*2 || prob(30))
						child.germ_level++

		if (parent)
			if (parent.germ_level < germ_level && !(parent.status & ORGAN_ROBOTIC))
...
/obj/item/organ/external/proc/update_wounds()
	...
	if((status & ORGAN_ROBOTIC)) //Robotic limbs don't heal or get worse.
...
/obj/item/organ/external/proc/update_damages()
	...
	for(var/datum/wound/W in wounds)
		...
		if(!(status & ORGAN_ROBOTIC) && W.bleeding() && (H && H.should_have_organ(BP_HEART)))
			W.bleed_timer--
			status |= ORGAN_BLEEDING
...
/obj/item/organ/external/proc/droplimb(var/clean, var/disintegrate = DROPLIMB_EDGE, var/ignore_children, var/silent)
	...
	switch(disintegrate)
		if(DROPLIMB_EDGE)
			if(!clean)
				var/gore_sound = "[(status & ORGAN_ROBOTIC) ? "tortured metal" : "ripping tendons and flesh"]"
		...
		if(DROPLIMB_BURN)
			var/gore = "[(status & ORGAN_ROBOTIC) ? "": " of burning flesh"]"
			owner.visible_message(
		...
		if(DROPLIMB_BLUNT)
			var/gore = "[(status & ORGAN_ROBOTIC) ? "": " in shower of gore"]"
	...
	if(owner)
		...
		if(!keep_organs)
			for(var/obj/item/organ/thing in internal_organs)
				if(istype(thing))
					if(thing.vital || thing.status & ORGAN_ROBOTIC)
						continue
...
/obj/item/organ/external/proc/is_malfunctioning()
	return ((status & ORGAN_ROBOTIC) && (brute_dam + burn_dam) >= 10 && prob(brute_dam + burn_dam))
...
/obj/item/organ/external/removed(var/mob/living/user, var/ignore_children = 0)
	...
	if(!owner)
		return
	var/is_robotic = status & ORGAN_ROBOTIC
...
/obj/item/organ/external/proc/jointlock(mob/attacker)
	...
	var/armor = owner.run_armor_check(owner, DAM_BLUNT)
	if(armor < 100)

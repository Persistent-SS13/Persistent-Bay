...
//	S.cd = GLOB.using_map.character_save_path(default_slot)
	...
	if(faction)
		...
		id.associated_account_number = M.account_number // All players get their acct connected to their ID
		if(record2)
		...
		if(stack)
			stack.connected_faction = faction.uid
			stack.try_connect()
		if(faction_uid == "nanotrasen")

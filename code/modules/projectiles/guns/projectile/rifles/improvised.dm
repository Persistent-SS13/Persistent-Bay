...
/obj/item/weapon/imprifleframe/examine(mob/user)
	...
	switch(buildstate)
		...
		if(2) user << "It has an unfinished pipe barrel taped in place."
		if(3) user << "It has an unfinished reinforced pipe barrel taped in place."
	...
	if(istype(W,/obj/item/pipe))
		...
		if(buildstate == 1)
			if(tape.use_tape(IMPROVRIFLE_TAPE_NEEDED))
				user << "<span class='notice'>You secure the barrel to the wooden furniture with [tape].</span>"
				buildstate++
				update_icon()
			else
				user << "<span class='notice'>You need at least [IMPROVRIFLE_TAPE_NEEDED] segments of [tape] to complete this task.</span>"
			return

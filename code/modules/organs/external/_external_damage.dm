...
/obj/item/organ/external/proc/is_damageable(var/additional_damage = 0)
	...
	return ((status & ORGAN_ROBOTIC) || brute_dam + burn_dam + additional_damage < max_health * 4)
...
/obj/item/organ/external/take_damage(damage = 0 as num, damtype = DAM_BLUNT, armorbypass = 0 as num, damsrc = null)
	...
	if(damage && damtype)
		...
		//can_cut = (prob(brute*2) || sharp) && (!(status & ORGAN_ROBOTIC))
		pure_brute = brute
...
/obj/item/organ/external/heal_damage(brute, burn, internal = 0, robo_repair = 0)
	if(status & ORGAN_ROBOTIC && !robo_repair)
		return
...
/obj/item/organ/external/proc/get_genetic_damage()
	return ((species && (species.species_flags & SPECIES_FLAG_NO_SCAN)) || status & ORGAN_ROBOTIC) ? 0 : genetic_degradation

/obj/item/organ/external/proc/remove_genetic_damage(var/amount)
	if((species.species_flags & SPECIES_FLAG_NO_SCAN) || status & ORGAN_ROBOTIC)
...
/obj/item/organ/external/proc/get_pain()
	if(!can_feel_pain() || status & ORGAN_ROBOTIC)
		return 0
	var/lasting_pain = 0
	if(is_broken())
		lasting_pain += 10
...
/obj/item/organ/external/proc/sever_artery()
	if(species && species.has_organ[BP_HEART])
		...
		if(!(status & ORGAN_ROBOTIC) && !(status & ORGAN_ARTERY_CUT) && !initial(O.open))
			status |= ORGAN_ARTERY_CUT
			return TRUE
	return FALSE

/obj/item/organ/external/proc/sever_tendon()
	if(has_tendon() && !(status & ORGAN_ROBOTIC) && !(status & ORGAN_TENDON_CUT))

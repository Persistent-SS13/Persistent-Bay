/obj/mecha/combat
	...
	damage_absorption = list(DAM_BLUNT = 0.7, DAM_PIERCE = 0.65, DAM_CUT = 0.7, DAM_BURN = 1, DAM_BULLET = 0.7, DAM_LASER = 0.85, DAM_ENERGY = 1, DAM_BOMB = 0.8)
	...
	if(istype(target, /mob/living))
		...
		if(src.occupant.a_intent == I_HURT)
			...
			*/
			...
			switch(damtype)
				if(DAM_BLUNT)
					var/blocked = M.run_armor_check(hit_zone, DAM_BLUNT)
					if(M.apply_damage(rand(force/2, force), DAM_BLUNT, hit_zone, blocked))
						M.Weaken(1)
				if(DAM_BURN)
					var/blocked = M.run_armor_check(hit_zone, DAM_BURN)
					M.apply_damage(rand(force/2, force), DAM_BURN, hit_zone, blocked)
				if(DAM_BIO)
					if(M.reagents)

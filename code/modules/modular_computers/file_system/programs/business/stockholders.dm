	...
	if(connected_faction.get_stockholder(user.real_name))
		...
		if(menu == 4)
			if(!selected_proposal)
				menu = 1
				return 1
			var/list/formatted_support[0]
			for(var/datum/stockholder/holder in selected_proposal.supporting)
				formatted_stockholders[++formatted_stockholders.len] = list("name" = "[x] ([holder.stocks])")
	...
	switch(href_list["action"])
		...
		if("fire_ceo")
			...
			if(connected_faction.leader_name && connected_faction.leader_name != "")
				...
				to_chat(usr, "Their is currently no CEO to fire.")
		if("choose_ceo")
			var/select_name = sanitizeName(input(usr,"Enter the full name of the new CEO appointee.","CEO name", "") as null|text, MAX_NAME_LEN, 1, 0,1)	
			if(connected_faction.has_proposal(user.real_name))
				to_chat(user, "You can only have one active proposal at a time.")
				return 1
			if(select_name)
		...
		if("ceo_wage")
			var/new_pay = input("Enter new proposed CEO wage.","CEO Wage") as null|num
		...
		if("ceo_revenue")
			var/new_pay = input("Enter new proposed CEO revenue share. 30% Max","CEO Revenue Share") as null|num
		...
		if("stockholder_revenue")
			var/new_pay = input("Enter new proposed stockholder revenue share. 30% Max","Stockholder Revenue Share") as null|num
			if(!new_pay && new_pay != 0 || new_pay > 30) return 1
			if(connected_faction.has_proposal(user.real_name))
				to_chat(user, "You can only have one active proposal at a time.")
				return 1
		...
		if("instant_dividend")
			var/new_pay = input("Enter percentage of the current account balance to divide up for the dividend.","Instant Dividend") as null|num

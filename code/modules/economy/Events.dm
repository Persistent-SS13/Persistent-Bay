...
/datum/event/economic_event/start()
	...
	if(affected_dest.viable_random_events.len)
		...
		switch(event_type)
			if(EC_RIOTS)
				dearer_goods = list(EC_SECURITY)
				cheaper_goods = list(EC_MINERALS, EC_FOOD)
			if(EC_WILD_ANIMAL_ATTACK)
				cheaper_goods = list(EC_ANIMALS)
				dearer_goods = list(EC_FOOD, EC_BIOMEDICAL)
			if(EC_INDUSTRIAL_ACCIDENT)
				dearer_goods = list(EC_EMERGENCY, EC_BIOMEDICAL, EC_ROBOTICS)
			if(EC_BIOHAZARD_OUTBREAK)
				dearer_goods = list(EC_BIOMEDICAL, EC_GAS)
			if(EC_PIRATES)
				dearer_goods = list(EC_SECURITY, EC_MINERALS)
			if(EC_CORPORATE_ATTACK)
				dearer_goods = list(EC_SECURITY, EC_MAINTENANCE)
			if(EC_ALIEN_RAIDERS)
				dearer_goods = list(EC_BIOMEDICAL, EC_ANIMALS)
				cheaper_goods = list(EC_GAS, EC_MINERALS)
			if(EC_AI_LIBERATION)
				dearer_goods = list(EC_EMERGENCY, EC_GAS, EC_MAINTENANCE)
			if(EC_MOURNING)
				cheaper_goods = list(EC_MINERALS, EC_MAINTENANCE)
			if(EC_CULT_CELL_REVEALED)
				dearer_goods = list(EC_SECURITY, EC_BIOMEDICAL, EC_MAINTENANCE)
			if(EC_SECURITY_BREACH)
				dearer_goods = list(EC_SECURITY)
			if(EC_ANIMAL_RIGHTS_RAID)
				dearer_goods = list(EC_ANIMALS)
			if(EC_FESTIVAL)
				dearer_goods = list(EC_FOOD, EC_ANIMALS)

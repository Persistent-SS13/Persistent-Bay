...
/obj/item/weapon/tool/weldingtool/attack(mob/living/M, mob/living/user, target_zone)
	if(ishuman(M))
		...
		if(!S || !(S.status & ORGAN_ROBOTIC) || user.a_intent != I_HELP)
			return ..()
...
/obj/item/weapon/welder_tank/examine(mob/user)
	if(..(user, 0))
		to_chat(user, "<span class='notice'>There is [reagents.total_volume]/[tank_volume] units of fuel remaining.</span>")

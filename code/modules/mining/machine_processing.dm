...
/obj/machinery/mineral/processing_unit/Bumped(var/atom/movable/A)
		...
		if(ores_stored[metal] > 0 && ores_processing[metal] != 0)
			...
			if(ores_processing[metal] == 3 && O.alloy) //Alloying.
				...
				for(var/datum/alloy/A in alloy_data)
					...
					if(!enough_metal)
						...
						for(var/i=0,i<total,i++)
							new A.product(get_step(loc, dir))
				...
				for(var/i=0,i<can_make,i+=2)
					ores_stored[metal]-=2
					sheets+=2
					new M.stack_type(get_step(loc, dir))
				...
				ores_stored[metal]--
				sheets++
				new /obj/item/weapon/ore/slag(get_step(loc, dir))
		else
			continue
	updateUsrDialog()

/**********************Mineral processing unit console**************************/
/obj/machinery/mineral/processing_unit/attack_hand(mob/user)
	add_fingerprint(user)
	interact(user)

/obj/machinery/mineral/processing_unit/interact(mob/user)
	...
	for(var/ore in ores_processing)
		...
		if(!ores_stored[ore] && !show_all_ores) continue
		var/ore/O = ore_data[ore]
		if(!O) continue
		dat += "<tr><td width = 40><b>[capitalize(O.display_name)]</b></td><td width = 30>[ores_stored[ore]]</td><td width = 100>"
	...
	dat += "The ore processor is currently <A href='?src=\ref[src];toggle_power=1'>[(active ? "<font color='green'>processing</font>" : "<font color='red'>disabled</font>")]</a>."
	var/datum/browser/popup = new(usr, "processor_console", "Ore Processing Console", 400, 500)
